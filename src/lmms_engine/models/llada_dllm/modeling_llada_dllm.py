...
@dataclass
class LLaDADLLMMaskedLMOutput(MaskedLMOutput):
...
class LLaDADLLMRMSNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6):
        """
        LLaDADLLMRMSNorm is equivalent to T5LayerNorm
        """
        super().__init__()
...
class LLaDADLLMAttention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""

    def __init__(self, config: LLaDADLLMConfig, layer_idx: int):
        super().__init__()
        self.config = config
        ...
        self.q_norm = LLaDADLLMRMSNorm(self.head_dim, eps=config.rms_norm_eps)  # unlike olmo, only on the head dim!
        self.k_norm = LLaDADLLMRMSNorm(self.head_dim, eps=config.rms_norm_eps)  # thus post q_norm does not need reshape
...
class LLaDADLLMDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: LLaDADLLMConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size

        self.self_attn = LLaDADLLMAttention(config=config, layer_idx=layer_idx)

        self.mlp = LLaDADLLMMLP(config)
        self.input_layernorm = LLaDADLLMRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = LLaDADLLMRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class LLaDADLLMPreTrainedModel(PreTrainedModel):
    config: LLaDADLLMConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["LLaDADLLMDecoderLayer"]
    ...
    _can_record_outputs = {
        "hidden_states": LLaDADLLMDecoderLayer,
        "attentions": LLaDADLLMAttention,
    }


class LLaDADLLMRotaryEmbedding(nn.Module):
    def __init__(self, config: LLaDADLLMConfig, device=None):
        super().__init__()
...
class LLaDADLLMModel(LLaDADLLMPreTrainedModel):
    def __init__(self, config: LLaDADLLMConfig):
        ...
        self.layers = nn.ModuleList(
            [LLaDADLLMDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.norm = LLaDADLLMRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.rotary_emb = LLaDADLLMRotaryEmbedding(config=config)
...
class LLaDADLLMForMaskedLM(LLaDADLLMPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = LLaDADLLMModel(config)
        self.vocab_size = config.vocab_size
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if reduction == "sum":
            ...
            d_loss = d_loss.mean()
            nll = nll.mean()
        return LLaDADLLMMaskedLMOutput(
            loss=d_loss,
            nll=nll,
            logits=logits,
            hidden_states=hidden_states,
