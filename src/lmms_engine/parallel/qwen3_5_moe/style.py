"""ParallelStyle for Qwen3_5MoeExperts.

Identical EP dispatch/permute/combine logic as Qwen3MoeParallelStyle; only the
class check for partition_fn differs.
"""
...
class Qwen3_5MoeParallelStyle(ParallelStyle):
    def __init__(
        self,
    ...
    def _partition_fn(name, mod, device_mesh):
        if isinstance(mod, Qwen3_5MoeExperts):
            expert_parallel_dim = 0
            mod.register_parameter(
                "gate_up_proj",
                nn.Parameter(distribute_tensor(mod.gate_up_proj, device_mesh, [Shard(expert_parallel_dim)])),
            )
            mod.register_parameter(
                "down_proj",
                nn.Parameter(distribute_tensor(mod.down_proj, device_mesh, [Shard(expert_parallel_dim)])),
            )
    ...
    def _apply(self, module: nn.Module, device_mesh: DeviceMesh) -> nn.Module:
        ...
        return distribute_module(
            module,
            device_mesh,
            partition_fn=Qwen3_5MoeParallelStyle._partition_fn,
