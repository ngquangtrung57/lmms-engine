...
class TestQwen3VLMoe(TestCase):
    @with_temp_dir
    @with_multi_gpu_training
    def test_train_fsdp2(self, temp_dir, nproc_per_node):
        """Test Qwen3 VL MoE training with FSDP2 and Expert Parallelism using torchrun subprocess."""

        # Path to the training script
        script_path = os.path.join(os.path.dirname(__file__), "train_qwen3_vl_moe_ep.py")
    ...
    def test_train_fsdp2_sp(self, temp_dir, nproc_per_node):
        """Test Qwen3 VL MoE training with FSDP2 and sequence parallelism using torchrun subprocess."""

        # Path to the training script
        script_path = os.path.join(os.path.dirname(__file__), "train_qwen3_vl_moe_sp.py")
