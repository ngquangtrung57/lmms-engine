...
class TestQwen3_5Moe(TestCase):
    @with_temp_dir
    @with_multi_gpu_training
    def test_train_fsdp2(self, temp_dir, nproc_per_node):
        """Test Qwen3.5 MoE training with FSDP2 and Expert Parallelism using torchrun subprocess."""

        script_path = os.path.join(os.path.dirname(__file__), "train_qwen3_5_moe_ep.py")
